            ...
            when (chartViewState.value) {
                is ChartViewState.Simple -> {
                    panelToolParameters.value = panelToolParameters.value?.copy(
                        ...
                        instrumentItems = panelToolParameters.value?.instrumentItems?.map { it.copy(isSelected = false) }
                            ?: emptyList()
                    )
                }
                else -> {
            ...
            is ChartViewState.Simple -> {
                ...
                val newToolParameters = panelToolParameters.value?.copy(
                    instrumentItems = instrumentItems.map {
                        it.copy(
                            isSelected = (it == item && it.instrument != Instrument.SETTINGS && it.instrument != Instrument.DRAWING_TOOL)
                        )
                    }
                )
